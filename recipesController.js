...
exports.deleteRecipe = async (req, res) => {
    try {
        ...
        console.log(`Starting nuke sequence for Recipe ${recipeId} (User ${userId})`);

        console.log("Removing recipe from meal plans...");
        await pool.query("DELETE FROM meal_plans WHERE recipe_id = ?", [recipeId]);

        console.log("Removing comments and ratings...");
        ...
        if (result.affectedRows === 0) {
            console.warn("Delete failed: Recipe not found or unauthorized.");
            return res.status(403).json({ error: "Unauthorized or not found" });
        }

        console.log("Successfully deleted recipe and all associations.");
